...
def ProxNestedSampling(X0, LikeliL, proxH, proxB, params, options):
    ...
    driftIniN = lambda X, delta, lamb : np.real((1-delta/(2*lamb))*X + delta/(2*lamb)*proxH(X,lamb))
    drift = lambda X, delta, lamb, tau : np.real((1-delta/lamb)*X + delta/(2*lamb)*(proxH(X,lamb) + proxB(X, np.sqrt(tau*2*sigma**2))))

    # Initialize variables
    delta = options['delta']         # delta controls the proposal variance, the step-length and Moreau approximation
    ...
    for ii in tqdm(range(200), desc= 'ProxNest || Initialise'):
        ...
        Xcur = drift(Xcur, delta, lamb, tau_0) + np.sqrt(delta)*np.random.randn(Xcur.shape[0], Xcur.shape[1])


    # Obtain samples from priors
    for ii in tqdm(range(2, NumLiveSetSamples*options['thinning'] + options['burn']), desc= 'ProxNest || Populate'):

        # P-ULA -- MARKOV CHAIN generating live samples 
        Xcur = driftIniN(Xcur,delta,lamb) + np.sqrt(delta)*np.random.randn(Xcur.shape[0], Xcur.shape[1])
        
        # Save sample (with thinning)
        if (ii > options['burn']) and not ((ii - options['burn']) % options['thinning']):
    ...
    Xtrace['LiveSet'], Xtrace['LiveSetL'] = resampling.reorder_samples(Xtrace['LiveSet'], Xtrace['LiveSetL'])
    ...
    for k in tqdm(range(NumDiscardSamples), desc= 'ProxNest || Sample'):
        ...
        indNewSample = np.floor(np.random.rand()*(NumLiveSetSamples-1)).astype(int) - 1
        Xcur = Xtrace['LiveSet'][indNewSample]

        # Generate a new sample with likelihood larger than given threshould
        Xcur = drift(Xcur, delta, lamb, tau) + np.sqrt(delta)*np.random.randn(Xcur.shape[0], Xcur.shape[1])
    ...
    lg.info_log("Estimating Bayesian evidence (with variance), posterior probabilies, and posterior mean...")
    ...
    for i in tqdm(range(1, NumDiscardSamples-1), desc= 'ProxNest || Trapezium Integrate'):
        discardLen[i] = (Xtrace['DiscardW'][i-1] - Xtrace['DiscardW'][i+1])/2

    discardLen[-1] = (np.exp(-(NumDiscardSamples-1)/NumLiveSetSamples) - np.exp(-(NumDiscardSamples+1)/NumLiveSetSamples))/2; # volume length of the last discarded sample 

    liveSampleLen = np.exp(-(NumDiscardSamples)/NumLiveSetSamples); # volume length of the living sample
    ...
    for k in tqdm(range(NumDiscardSamples), desc= 'ProxNest || Compute Posterior Mean'):
        Xtrace['DiscardPostProb'][k] = np.exp(Xtrace['DiscardL'][k] + np.log(discardLen[k]) - BayEvi[0]); 
