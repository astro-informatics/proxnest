...
for i in dirs:
    if df.empty:
        df = pd.read_csv(i+"/predictions.csv", names=("dimension", "ground truth", f"predictions {n}"))
    else:
        preds = pd.read_csv(i+"/predictions.csv", names=("dimension", "ground truth", f"predictions {n}"))
...
plt.figure(dpi=300)
plt.errorbar(dims, avgs, yerr=stds, marker="x", capsize=2, 
             linewidth=1, elinewidth=0.5, color="red", ecolor="blue")
