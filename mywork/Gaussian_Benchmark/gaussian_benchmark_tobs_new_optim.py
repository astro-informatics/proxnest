...
def main(args):
    ...
    for it_run in range(args.runs):
        ...
        warm_start_coeff = 1e-1,                  # Warm start coefficient
            ...
            proxB = lambda x, tau: optimisations.l2_ball_proj.new_sopt_fast_proj_B2(x, tau, params)
            ...
            BayEvi_Val_gt_log = 0.5 * dimension * np.log(2 * np.pi * detPar) + (-ySquare/(2*sigma**2)) + (detPar/2)*(ySquare/sigma**4)
            ...
            array_predictions[it_run,it_dim,:] = np.array([dimension,BayEvi_Val_gt_log,rescaled_evidence_estimate])
    ...
    plt.ylim(0,np.max(mean_predictions[:,1:]))
