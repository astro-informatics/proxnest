...
def main(args):
    ...
    dimensions = np.append(dimensions, [400, 600, 800, 1000])
    ...
    for it_run in range(args.runs):
        ...
        for it_dim in range(dimensions.shape[0]):
            ...
            options = utils.create_options_dict(
                ...
                lv_thinning=5e0,  # Thinning factor in the sample update                        ## 1e1 in Cai et. al.
                MH_step=True,  # Metropolis-Hastings step
                warm_start_coeff=5e1,  # Warm start coefficient
    ...
    plt.savefig(save_dir + "plot_hd.pdf")
