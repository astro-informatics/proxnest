...
def plot(param_name, test_params):
    print(f"Plotting for {param_name}")
    print(f"Baseline = {defaults[param_name]}")
    color = cm.spring(np.linspace(0, 1, len(test_params)))
    plt.style.use("seaborn-v0_8-darkgrid")
    fig = plt.figure(dpi=500)
    ax = plt.subplot()
    baseline = pd.read_csv("/Users/henry/Work/PhD/Project/Repositories/proxnest/mywork/Gaussian_Benchmark/"+args.label+"/Baseline/mean_predictions.csv", names=("dim", "gt", "pred"))
    for n, value in enumerate(test_params):
        print(f"{param_name} = {value}")
        save_dir = "/Users/henry/Work/PhD/Project/Repositories/proxnest/mywork/Gaussian_Benchmark/"+args.label+"/"+param_name+"/"+str(value)+"/"
        if not os.path.isdir(save_dir):
            continue
        df = pd.read_csv(save_dir+"mean_predictions.csv",  names=("dim", "gt", "pred"))
        if n == 0:
            ax.plot(df["dim"], df["gt"], color='black', linewidth=1, label="GT")
            ax.plot(baseline["dim"], baseline["pred"], linewidth=1, linestyle="dashed", color="black", label="Baseline: {:.1e}".format(defaults[param_name]))
        ax.plot(df["dim"], df["pred"], color=color[n], markersize=2, marker="x", label="{:.1e}".format(value), linewidth=0.5)

    ax.set_ylim(-10,700)
    ax.set_xlim(0,1000)
    ax.set_xlabel("Dimensions")
    ax.set_ylabel(r"$\log (V \times \mathcal{Z})$")
    box = ax.get_position()
    ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    ax.set_title(f"Varying {param_name}")

    plt.savefig("/Users/henry/Work/PhD/Project/Repositories/proxnest/mywork/Gaussian_Benchmark/"+args.label+"/"+param_name+"/comparison_plot_HD.png")

    ax.set_ylim(-10,130)
    ax.set_xlim(0,200)
    plt.savefig("/Users/henry/Work/PhD/Project/Repositories/proxnest/mywork/Gaussian_Benchmark/"+args.label+"/"+param_name+"/comparison_plot.png")


def main(args):
    start = time.time()
    #dimensions = np.linspace(args.dims[0], args.dims[1], args.dims[2], dtype=int)
    #dimensions = np.linspace(20, 200, 5, dtype=int)
    dimensions = np.array([400, 600, 800, 1000], dtype=int)
    ...
    for it_run in range(args.runs):
        ...
        for it_dim in range(dimensions.shape[0]):
            ...
            print(f"RUN {it_run}: Calculating for dimension: {dimension}")
            ...
            options = utils.create_options_dict(
                ...
                lv_thinning_init=lv_thinning_init,  # Thinning factor in initialisation
                lv_thinning=lv_thinning,  # Thinning factor in the sample update                        ## 1e1 in Cai et. al.
                MH_step=True,  # Metropolis-Hastings step
                warm_start_coeff=warm_start_coeff,  # Warm start coefficient
                delta=1e-2,  # Discretisation stepsize                                     ## 10*1e-1 in src_proxnest
                lamb=lamb * 1e-2,  # Moreau-Yosida approximation parameter, usually `5 * delta`
    ...
    plt.ylim(0, np.max(mean_predictions[:5, 1:]) + 10)
    plt.xlim(0, 210)
    plt.xlabel("Dimensions")
    plt.ylabel(r"$\log (V \times \mathcal{Z})$")
    title_str = "runs = {:d}, samplesL = {:.1e}, samplesD = {:.1e}, MH_step = {}, delta = {:.1e}, \n lv_thinning_init = {:.1e}, lv_thinning = {:.1e}, warm_start_coeff = {:.1e}, lamb = {:.1e}, prior_regulariser = {:.1e},\n time = {:.1e}".format(
        args.runs,
        options["samplesL"],
        options["samplesD"],
        options["MH_step"],
    ...
    plt.xlim(0, 1010)
    plt.savefig(save_dir + "plotHD.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    defaults = {
        ...
        "lv_thinning": 5e0,
        "warm_start_coeff": 1e1,
        "lamb": 5,
        "prior_regulariser": 0.1
    }
    ...
    if args.p:
        ...
        lv_thinning_inits = list(np.logspace(np.log10(1e0), np.log10(1e3), num=10, endpoint=True, base=10.0, dtype=int))
        lv_thinning_inits.remove(10)

        lv_thinnings = list(np.linspace(1, 10, num=10, dtype=int))
        lv_thinnings.remove(1)

        warm_start_coeff_list = list(np.logspace(np.log10(1e0), np.log10(1e2), num=10, endpoint=True, base=10.0))

        lambs = [1,2,3,4,6,7,8,9,10]

        prior_regularisers = list(np.linspace(0.05, 0.15, num=10))

        params = {
            "lv_thinning_init": lv_thinning_inits,
            "lv_thinning": lv_thinnings,
            "warm_start_coeff": warm_start_coeff_list,
            "lamb": lambs,
            "prior_regulariser": prior_regularisers
        }
        ...
        for param_name in args.p:
            ...
            #     df = pd.read_csv(save_dir+"mean_predictions.csv",  names=("dim", "gt", "pred"))
            #     save_dir = "/Users/henry/Work/PhD/Project/Repositories/proxnest/mywork/Gaussian_Benchmark/"+args.label+"/"+param_name+"/"
            #     if not os.path.isdir(save_dir):
            #         os.makedirs(save_dir)
            #     if n == 0:
            #         plt.plot(df["dim"], df["gt"], color='black')
            #         plt.plot(baseline["dim"], baseline["pred"], linestyle="dashed", color="black")
            #     plt.plot(df["dim"], df["pred"], color=colors[n], markersize=2, linewidth=0.5, marker="x", label=str(round(p,2)))
            ...
            # plt.ylabel(r"$\log (V \times \mathcal{Z})$")
            # plt.legend()
            # plt.title(f"Varying {param_name}")
            # plt.savefig(save_dir+"comparison_plot.pdf")
